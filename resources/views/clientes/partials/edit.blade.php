...
<div x-show="modalEditarCliente"
    ...
    <div x-show="modalEditarCliente"
         @click.away="modalEditarCliente = false"
        ...
        <div class="px-8 py-6 border-b border-slate-100 dark:border-zinc-800/60 flex justify-between items-center bg-white dark:bg-zinc-900 shrink-0">
            <h3 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">
                Editar <span class="text-[#818CF8]">Cliente</span>
            </h3>
            <button type="button" @click="modalEditarCliente = false" class="p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-xl transition-all duration-200 focus:outline-none">
        ...
        <template x-if="clienteEditando">
            ...
            <div class="overflow-y-auto custom-scrollbar p-8">
                <form :action="'/clientes/' + clienteEditando.id" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-6 mt-2 border-t border-slate-100 dark:border-zinc-800/60 shrink-0">
                        <button type="button" @click="modalEditarCliente = false" class="px-6 py-2.5 text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-xl transition-all duration-300">
                            Cancelar
                        </button>
